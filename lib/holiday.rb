...
holiday_hash.each do |season, holiday_of_hashes|
  ...
  holiday_of_hashes.each do |holiday, supplies_array|
    puts "  #{holiday.to_s.split("_").collect { |x| x.capitalize }.join(" ")}: #{supplies_array.capitalize.join(", ")}"
  end 
end 
 end 
